...
const BottomNavBar = () => {
    ...
    const navItems = [
        { href: "/", icon: homeIcon, label: "Home", activeIcon: activeHomeIcon },
        {
            ...
            label: "Order History",
            activeIcon: activeOrderHistoryIcon,
        },
    ...
    return (
        <div className="fixed grid grid-cols-4 bottom-0 left-0 right-0 bg-white shadow-md border-t border-gray-200 px-4 py-2  md:hidden">
            {navItems.map((item) => (
                <Link
                    key={item.href}
                    href={item.href}
